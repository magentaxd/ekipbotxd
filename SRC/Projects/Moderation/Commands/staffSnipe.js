...
if (!message.member.hasPermission("ADMINISTRATOR")) return;
  let data = serverdb.cek(`snipe.${message.guild.id}`);
  if(!data) return message.react(emojis.hata)
  moderationfx.embedOlustur(message.author, message.channel, `Yazan Kişi: <@${data.sahip}>
  Mesaj: \`${data.icerik}\`
  Kanal: <#${data.kanal}>`, "RANDOM").then(x => x.delete({timeout: 15000}));
  serverdb.sil(`snipe.${message.guild.id}`)

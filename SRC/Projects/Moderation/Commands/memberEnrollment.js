...
module.exports.execute = async(ModerationClient , message, args) => {
 let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter(settings.footer).setColor("RANDOM");
   ...
   let soncezainfo = penals.cek(`sonceza.${member.id}.${settings.server}`);
     ...
     if (!sicilminerdelanit) return moderationfx.embedHata(message.author, message.channel, `**${message.guild.name}** sunucusunda kayıtlı ${member} kullanıcısının sicil kayıdı bulunmuyor.`)
